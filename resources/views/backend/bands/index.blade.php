    ...
    <div class="container">
        <div class="card mt-5">
            ...
            <div class="mt-1 text-end">
                <a href="{{ route('bands.create') }}" class="btn btn-primary">Create New Band</a>
            </div>
            <div class="card-body">
...
<script>
    $(document).ready(function(){
        ...
        var table = $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('bands.index') }}", // Corrected route name
            columns: [
                { data: 'id', name: 'id' },
        ...
        $(document).on('click', '.delete', function() {
            ...
            if (confirm("Are you sure you want to delete this brand?")) {
                $.ajax({
                    type: "DELETE",
                    url: "{{ route('bands.destroy', '') }}/" + id, // Corrected route name
                    data: {
                        _token: "{{ csrf_token() }}"
                    },
